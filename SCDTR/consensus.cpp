...
float Consensus::iterate(void) {
    ...
    for (int i = 0; i < n_rpi; i++) {
        d_bl[i] = z[i] / rho -
                  (gain[i] / n) * (o - L + (1 / rho) * vec_mult(z, gain));
    }
    ...
    if(occupied_flag == true) d_b0[index] = occupied_lower_boundary;
    else d_b0[index] = unoccupied_lower_boundary;
    ...
    for (int i = 0; i < n_rpi; i++) {
        d_l0[i] = z[i] / rho - (gain[i] / m) * (o - L) +
                  (gain[i] / (rho * m)) *
                      (gain[index] * z[index] - vec_mult(z, gain));
    }
    if (occupied_flag == true)
        d_l0[index] = occupied_lower_boundary;
    else
        d_l0[index] = unoccupied_lower_boundary;
    ...
    for (int i = 0; i < n_rpi; i++) {
        d_l1[i] = z[i] / rho - (gain[i] / m) * (o - L + 100 * gain[index]) +
                  (gain[i] / (rho * m)) *
                      (gain[index] * z[index] - vec_mult(z, gain));
    }
    d_l1[index] = 100.0;
...
float Consensus::get_boundary_unoccupied(void) {
    return unoccupied_lower_boundary;
}
float Consensus::get_cost(void) { return c[index]; }

// Modify
void Consensus::set_cost(float val) { c[index] = val; }
...
void Consensus::set_uboundary(float val) {
    if (val > occupied_lower_boundary)
        unoccupied_lower_boundary = occupied_lower_boundary;
    else if (val < 0.0)
        unoccupied_lower_boundary = 0.0;
    else if (val > 100.0)
        unoccupied_lower_boundary = 100.0;
    else
        unoccupied_lower_boundary = val;
}
void Consensus::set_oboundary(float val) {
    if (val < occupied_lower_boundary)
        occupied_lower_boundary = unoccupied_lower_boundary;
    else if (val < 0.0)
        occupied_lower_boundary = 0.0;
    else if (val > 100.0)
        occupied_lower_boundary = 100.0;
    else
        occupied_lower_boundary = val;
}
